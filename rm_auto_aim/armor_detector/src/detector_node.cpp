...
std::unique_ptr<Inference> ArmorDetectorNode::initDetector(const std::string& modelpath)
{
  ...
  auto model_path = pkg_path + "/model/yolov8.onnx";

  auto detector = std::make_unique<Inference>(model_path, detect_color);

  return detector;
}

std::vector<Detection> ArmorDetectorNode::detectArmors(
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  auto armors = detector_->runInference(img);
